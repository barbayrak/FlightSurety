...
(async() => {
    ...
    let contract = new Contract('localhost', () => {
        ...
        DOM.elid("operational-status-get").addEventListener("click", async () => {            
            ...
            let err, result;
            try {
                result = await contract.isOperational(request);
            } catch (e) {
                console.log(e);
                err = e;
            } finally {
                if(result){
                    display('Operational Status', 'Check if contract is operational', [ { label: 'Operational Status', error: err, value: "Operational"} ]);
                }else{
                    display('Operational Status', 'Check if contract is operational', [ { label: 'Operational Status', error: err, value: "Not Operational"} ]);
                }
        ...
        DOM.elid("airline-register").addEventListener("click", async () => {
            ...
            let airlineName = DOM.elid("airline-name");
            let from = DOM.elid("airline-register-from");
            let request = {
                airlineAddress: airlineAddress.value,
        ...
        DOM.elid("register-flight").addEventListener("click", async ()=>{
            let request = {
                ...
                departure: new Date(DOM.elid("register-flight-departure").value).getTime(),
        ...
        DOM.elid("submit-oracle").addEventListener("click", async () => {
            ...
            let request = {
                ...
                departure: new Date(DOM.elid("submit-oracle-departure").value).getTime()
            };
            let err, result;
            try {
                ...
                display('Flight Status',
                    ...
                    [
                        { label: 'Flight Status :', error: err, value: "Updated"}
                    ]
                );
            }
        });
            ...
            try {
                ...
                display('Get Flight',
                    ...
                    [
                        ...
                        { label: "Departure Status", error: err, value: result.statusCode },
                        { label: "Departure", error: err, value: new Date(result.updatedTimestamp.toNumber() * 1000) }
                    ]
                );
            }
        });
        ...
        DOM.elid("get-insurance").addEventListener("click", async ()=>{
            let request = {
                passenger: DOM.elid("get-insurance-id").value
            };
            ...
            try {
                ...
                err = e;
                console.log(e);
                label = "Insurance not found for passenger : " + request.passenger;
            } finally {
                display('Get Insurance',
                    'Get flight id of insurance',
                    [
                        { label: "Passenger's insurance with flight id : ", error: err, value: result }
                    ]
                );
            }
        });
