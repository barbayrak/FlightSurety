...
export default class Contract {
    ...
    async registerAirline(request) {
        let caller = request.from || this.owner;
        let instance = await this.getContractInstance();
        return await instance.registerAirline(request.airlineAddress,request.airlineName, {from: caller});
    }
    async registerFlight(request){
        let caller = request.from || this.owner;
        let instance = await this.getContractInstance();
        return await instance.registerFlight(request.flight,request.airline,request.departure, {from: caller});
    }
    ...
    async getInsurance(request){
        console.log(request);
        let instance = await this.getContractInstance();
        let result = await instance.getInsurance(request.passenger);
        console.log(result);
        return result;
    }
